...
# 1. Сборка tramp.asm и генерация хедера (только если tramp.asm изменился)
TRAMP_CHANGED=0
...
# иначе disk.o останется со старым содержимым трамплина внутри.
for src in lib/*.c; do
    flat_name="${src//\//_}"
...
# 5. Линковка и создание бинарника (делаем всегда - дёшево по сравнению с компиляцией)
...
# 6. Сборка boot.asm - KERNEL_SECTORS зависит от размера kernel.bin, кеш тут не подходит
...
dd if=boot.bin of=disk.img conv=notrunc >/dev/nul
dd if=kernel.bin of=disk.img bs=512 seek=1 conv=notrunc >/dev/nul
